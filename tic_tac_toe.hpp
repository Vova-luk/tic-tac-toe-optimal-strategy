...
/**
 * @brief Сравнивает расстояние до конца партии для двух оценок позиции.
 *
 * Для выигрышной позиции игрок предпочитает более короткий путь к победе,
 * а для проигрышной позиции - более длинный путь до поражения. Для ничьей
 * расстояние не влияет на сравнение.
 *
 * @param[in] first Первая оценка позиции.
 * @param[in] second Вторая оценка позиции.
 * @param[in] strategy_turn `true`, если ход делает игрок стратегии.
 * @return `1`, если первая оценка предпочтительнее по расстоянию.
 * @return `0`, если расстояния считаются равнозначными.
 * @return `-1`, если первая оценка хуже по расстоянию.
 */
int CompareDistance(const PositionScore& first, const PositionScore& second, bool strategy_turn);

/**
 * @brief Сравнивает два целочисленных значения по заданному направлению
 *        предпочтения.
 *
 * Функция используется как универсальная: можно предпочитать
 * либо большее значение, либо меньшее.
 *
 * @param[in] first Первое значение.
 * @param[in] second Второе значение.
 * @param[in] prefer_greater `true`, если большее значение считается лучшим.
 * @return `1`, если первое значение предпочтительнее.
 * @return `0`, если значения равны.
 * @return `-1`, если первое значение хуже.
 */
